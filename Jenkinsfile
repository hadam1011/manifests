        ...
        stage ('Update deployment files') {
            steps {
                bat """
                    ...
                    (Get-Content Query-exporter-app/manifests/backend-deployment.yml) -replace 'imageVersion', ${BUILD_NUMBER} | Out-File -encoding ASCII Query-exporter-app/manifests/backend-deployment.yml
                    (Get-Content Query-exporter-app/manifests/frontend-deployment.yml) -replace 'imageVersion', ${BUILD_NUMBER} | Out-File -encoding ASCII Query-exporter-app/manifests/frontend-deployment.yml
                    git add .
